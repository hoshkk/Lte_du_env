...
- **Spectrum**: KT 밴드 프리셋(LTE B1/B3/B8, NR n78) 선택, TX/RX 전환(수신 주파수만 전환 — 실제
  송신 기능은 아래 "USB SDR 연동 > HackRF One > 송신(TX)" 참고), 마커 M1~M5, 피크 서치, 화면 탭으로
  마커 배치
...
- **송신(TX)**: HackRF One은 (RTL-SDR과 달리) 하드웨어 자체가 송수신 겸용 트랜시버라, 무변조 CW
  테스트 캐리어를 송신하는 기능을 넣었습니다. Settings에서 USB SDR로 HackRF가 연결되면 "Test
  Signal (TX)" 패널이 나타나고, 현재 Center 주파수에서 TX 게인(0~47dB)을 지정해 캐리어를 켜고 끌 수
  있습니다. **실제로 전파를 발사하는 기능**이니 안테나/더미로드 없이 직결로 쓰지 말고, 전파법상
  허가된 용도/출력으로만 사용하세요. RTL-SDR은 수신 전용 하드웨어라 이 패널이 아예 뜨지 않습니다
  (소프트웨어로 우회 불가능한 하드웨어 제약).
